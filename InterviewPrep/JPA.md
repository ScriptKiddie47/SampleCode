...
`@Transaction` -> Makes all operations commit together or roll back on failure. Can be defined on a class or method. If defined on a class all methods are transacitonal.
What if we call `transaction` `method` from a `non transactional one `in the same class ? - The Transaction won't start because spring AOP Proxy is not used.
Also can't be used on private methods.

1. `@Transient` – Field is NOT persisted to DB
1. `@Enumerated` – Map enum to DB column

N+1 Problem
- Due to @OneToMany between customers and addresses
- `@EntityGraph(attributePaths = "address") List<Customer> findAll();` or join query.

`Lazy` Loading - Load only the parent entity - Default Strategy
`Eager` Loading - Load the child entity as well
In a `Lazy` Loading scenario If you have getter & setter methods in Parent Entity. It will actually load the child entity in a second call unless you want a specific element of parent entity.`